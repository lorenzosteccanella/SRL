...
run = wandb.init(project='MAD_SemiNorm', entity='lsteccanella',
                 group="Planning_AsyReach_L1", settings=wandb.Settings(start_method="fork"))
wandb.run.name = "planning_asyreach_L1_" + str(seed)

env = gym.make('FetchReach-v2', max_episode_steps=100)
action_space = list(np.array(
  np.meshgrid(np.arange(-0.5, 1.2, 0.2), np.arange(-0.5, 1.2, 0.2), np.arange(-0.5, 1.2, 0.2), 0)).T.reshape(-1, 4))

np.random.seed(seed)
torch.manual_seed(seed)

model = torch.load(dirpath + "/Saved_models/AsyReach_L1_" + str(seed))
model.eval()
