...
run = wandb.init(project='MAD_SemiNorm', entity='lsteccanella',
                 group="Planning_AsyReach_WN", settings=wandb.Settings(start_method="fork"))
wandb.run.name = "planning_asyreach_WN_" + str(seed)
...
model = torch.load(dirpath + "/Saved_models/AsyReach_WN_" + str(seed))
model.eval()
