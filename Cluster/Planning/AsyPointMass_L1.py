...
run = wandb.init(project='MAD_SemiNorm', entity='lsteccanella',
                 group="Planning_AsyPointMass_L1", settings=wandb.Settings(start_method="fork"))
wandb.run.name = "planning_asypointmass_l1_" + str(seed)
...
model = torch.load(dirpath + "/Saved_models/AsyPointMass_L1_" + str(seed))
model.eval()
