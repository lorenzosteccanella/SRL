...
run = wandb.init(project='MAD_SemiNorm', entity='lsteccanella',
                 group="Planning_PointMass_WN", settings=wandb.Settings(start_method="fork"))
wandb.run.name = "planning_pointmass_WN_" + str(seed)
...
model = torch.load(dirpath + "/Saved_models/PointMass_WN_" + str(seed))
model.eval()
