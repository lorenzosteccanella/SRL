...
run = wandb.init(project='MAD_SemiNorm', entity='lsteccanella',
                 group="Planning_AsyPointMass_WN", settings=wandb.Settings(start_method="fork"))
wandb.run.name = "planning_asypointmass_WN_" + str(seed)
...
model = torch.load(dirpath + "/Saved_models/AsyPointMass_WN_" + str(seed))
model.eval()
