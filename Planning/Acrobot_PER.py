...
model.load_state_dict(
    torch.load("/home/lorenzo/Documenti/UPF/MDP_temporal_representation/Models/Acrobot_action_states_dictionary"))
model.eval()
z_goal_states = []
z_goal_states.append(model.encode_state(torch.FloatTensor([[-0.9661,  0.2581,  0.8875,  0.4607, -1.8354, -5.0000]])))
z_goal_states.append(model.encode_state(torch.FloatTensor([[-0.9957,  0.0925,  0.1404, -0.9901,  2.5899, -3.2061]])))
# z_goal_states.append(model.encode_state(torch.FloatTensor([[-0.5233, -0.8522, -0.3458, -0.9383, -1.1436,  0.1383]])))
# z_goal_states.append(model.encode_state(torch.FloatTensor([[-0.4499, -0.8931,  0.9800, -0.1991, -1.4202,  3.3455]])))
# z_goal_states.append(model.encode_state(torch.FloatTensor([[-0.5655, -0.8247, -0.4184, -0.9082, -1.0728, -0.1604]])))
# z_goal_states.append(model.encode_state(torch.FloatTensor([[0.8070, -0.5906, -0.8532, 0.5216, -0.7868, -10.4488]])))
# z_goal_states.append(model.encode_state(torch.FloatTensor([[0.9985,  0.0548,  0.5089, -0.8608, -1.7711, 13.9211]])))
# z_goal_states.append(model.encode_state(torch.FloatTensor([[0.9985,  0.0548,  0.5089, -0.8608, -1.7711, 13.9211]])))
...
wandb_group = "AcroBot_Planning"
wandb_name = "AcroBot_Planning_seed_" + str(seed)
...
def multi_step_lookahead(model, s, actions, max_n_rand_traj=5): # WARNING WARNING WARNING WARNING
    horizon = 5
