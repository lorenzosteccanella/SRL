# FROM https://github.com/spitis/deepnorms just used to define asym distance quasimetrics that respects the triangular
# inequality and are trainable with gradient descent
...
class WideNormMetric(nn.Module):
    def __init__(self,
        ...
        self.activation = ConcaveActivation(num_components,
                                            concave_activation_size) if concave_activation_size else nn.Identity()
...
class DeepNormMetric(nn.Module):
    def __init__(self, num_features, layers, activation=nn.ReLU, concave_activation_size=None, mode='avg',
                 symmetric=True):
        super().__init__()
        ...
        self.output_activation = ConcaveActivation(layers[-1],
                                                   concave_activation_size) if concave_activation_size else nn.Identity()
        self.reduce_metric = ReduceMetric(mode)
